import { Context } from '../src/backend/core/index.ts';
import { extension } from '../src/backend/extensions.ts';
import { getTaskbarItems } from '../src/backend/util/taskbarItems.ts';
...
const CLIENT_VISIBLE_FEATURE_FLAGS = new Set([
    'create_shortcut',
    'download_directory',
    ...
    async (req, res) => {
        ...
        const details = {
            ...
            human_readable_age: user.timestamp
                ? timeago.format(new Date(user.timestamp))
                : null,
        };
        ...
        if (user.last_activity_ts) {
            try {
                details.last_activity_ts = Math.round(
                    new Date(user.last_activity_ts).getTime() / 1000,
                );
            } catch {
                /* ignore parse error */
            }
